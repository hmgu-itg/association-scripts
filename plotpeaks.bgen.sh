...
echo Adding software dependencies...
module add $(module avail 2>&1 | grep '/plink/' | grep latest | sed 's/.latest.//')
module add $(module avail 2>&1 | grep '/bcftools/' | grep latest | sed 's/.latest.//')
module add $(module avail 2>&1 | grep '/gemma/' | grep latest | sed 's/.latest.//')
module add $(module avail 2>&1 | grep '/bedtools/' | grep latest | sed 's/.latest.//')


declare -a files=($(echo $files | tr ',' ' '))
declare -a samples=($(echo $samples | tr ',' ' '))
...
curpeak_p=1
first=1
$cat $assocfile | awk -v p=$pvalcoli -v signif=$signif '$p<signif'| sort -k${chrcoli},${chrcoli}n -k ${pscoli},${pscoli}n  | tr ';' '_' | while read line; 
do
...
a2coli=$(grep -w $a2col <(paste <(seq 1 $($cat $assocfile | head -n1 | tr '\t' '\n'| wc -l)) <($cat $assocfile | head -n1 | tr '\t' '\n')) | cut -f1)
	...
	if [[ "$curchr" -ne "$oldchr"  ||  "$curps" -gt "$(($oldpos+$flank_bp))" ]]
		...
		if [ $first -ne 1 ]	
			...
			echo "chr"${curpeak_c}:${curpeak_ps}-${curpeak_ps}_${curpeak_a1}_${curpeak_a2}
			perl ~ag15/VarAnnot.pl <(echo "chr"${curpeak_c}:${curpeak_ps}-${curpeak_ps}_${curpeak_a1}_${curpeak_a2}) > $curpeak_c.$curpeak_ps.line
			...
			for id in "${ids[@]}"
			do 
				echo plink --memory 15000 --bgen ${files[$id]} --sample ${samples[$id]} --chr $curpeak_c --from-bp $(($curpeak_ps-$flank_bp)) --to-bp $(($curpeak_ps+$flank_bp)) --out $id --make-bed				
				plink --memory 15000 --bgen ${files[$id]} --sample ${samples[$id]} --chr $curpeak_c --from-bp $(($curpeak_ps-$flank_bp)) --to-bp $(($curpeak_ps+$flank_bp)) --out $id --make-bed				
			...
			a1coli=$(grep -w $a1col <(paste <(seq 1 $(cat $assocfile | head -n1 | tr ' ' '\n'| wc -l)) <(cat $assocfile | head -n1 | tr ' ' '\n')) | cut -f1)
			a2coli=$(grep -w $a2col <(paste <(seq 1 $(cat $assocfile | head -n1 | tr ' ' '\n'| wc -l)) <(cat $assocfile | head -n1 | tr ' ' '\n')) | cut -f1)
			#cat <(echo $(head -n1 peakdata.ld) id gene consequence) <(join -1 3 -2 1 -a 1 <(sort -k3,3 peakdata.ld) <(~/getrsid.pl <(awk '$NF>0.1' peakdata.ld | tail -n+2 | cut -f2-5 -d' ' )| sort -k1,1) | grep -v MARKER)| awk '{if(NF<22){$21="NA";$22="NA";$23="NA"}if(NR>1){un=$1;deux=$2;trois=$3;$1=deux;$2=trois;$3=un;}print}' > peakdata.ld.annotated
			expnumcol=$(($(head -n1 peakdata.ld | tr ' ' '\n' | wc -l)+3))
			echo JOIN2
			join -a 1 -1 $pscoli -2 1 --header <(cat <(head -n1 peakdata.ld) <(tail -n+2 peakdata.ld | sort -k$pscoli,$pscoli)) <( cat <(echo $pscol id gene consequence) <(~ag15/getrsid.pl <(awk '$NF>0.1' peakdata.ld | tail -n+2 | awk -v chr=$chrcoli -v ps=$pscoli -v a1=$a1coli -v a2=$a2coli '{print $chr,$ps,$a1,$a2}') | sort -k1,1))  |awk -v en=$expnumcol '{if(NF<en){$(en-2)="NA";$(en-1)="NA";$en="NA"}print}'> peakdata.ld.annotated
			~ag15/get_phenotype.pl <(cut -f$(($expnumcol-2)) -d' ' peakdata.ld.annotated | grep -v -e NA -e novel -e id) | sed 's/ /&/g'| sed 's/\t/\t"/;s/$/"/' > phenotypes
...
for id in "${ids[@]}"
do 
echo plink --bgen ${files[$id]}  --sample ${samples[$id]} --chr $curpeak_c --from-bp $(($curpeak_ps-$flank_bp)) --to-bp $(($curpeak_ps+$flank_bp)) --out $id --make-bed
plink --bgen ${files[$id]}  --sample ${samples[$id]} --chr $curpeak_c --from-bp $(($curpeak_ps-$flank_bp)) --to-bp $(($curpeak_ps+$flank_bp)) --out $id --make-bed
awk 'OFS="\t"{if($2!~/:/ && $2!~/rs/){$2="chr"$1":"$4}print}' $id.bim  | tr ';' '_'> t
mv t $id.bim
...
echo
echo
echo /nfs/team144/software/locuszoom-1.3/locuszoom/bin/locuszoom --metal peakdata --refsnp "$refsnp" --markercol "$rscol" --pvalcol "$pvalcol" --db $curpeak_c.$curpeak_ps.db --prefix $curpeak_c.$curpeak_ps.500kb --plotonly showAnnot=T showRefsnpAnnot=T annotPch="21,24,24,25,22,22,8,7" rfrows=20 geneFontSize=.4 --ld merged.ld --start=$(($curpeak_ps-$flank_bp)) --end=$(($curpeak_ps+$flank_bp)) --chr=$curpeak_c showRecomb=T --delim ' '
/nfs/team144/software/locuszoom-1.3/locuszoom/bin/locuszoom --metal peakdata --refsnp "$refsnp" --markercol "$rscol" --pvalcol "$pvalcol" --db $curpeak_c.$curpeak_ps.db --prefix $curpeak_c.$curpeak_ps.500kb --plotonly showAnnot=T showRefsnpAnnot=T annotPch="21,24,24,25,22,22,8,7" rfrows=20 geneFontSize=.4 --ld merged.ld --start=$(($curpeak_ps-$flank_bp)) --end=$(($curpeak_ps+$flank_bp)) --chr=$curpeak_c showRecomb=T --delim ' '
