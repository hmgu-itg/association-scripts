...
# DATA PATHS
phenotype=$1
pheno_path="/nfs/t144_helic/Phenotypes/Arthur/source_file/HP/Residuals-Plots"
#matrix_bfile="/lustre/scratch114/projects/helic/assoc_freeze_Summer2015_POMAK/matrices/ldprune_indep_50_5_2.hwe.1e-5/output/1xseq.prune.hwe.matrix.cXX.txt"
matrix_bfile="/lustre/scratch114/projects/helic/assoc_freeze_Summer2015_POMAK/matrices/ldprune_indep_50_5_2.hwe.1e-5/output/1xseq.nomono.pruned.hwe.matrix.cXX.txt"
input_bfiles="/lustre/scratch114/projects/helic/assoc_freeze_Summer2015_POMAK/input/assoc_chunks"
...
tail -n+2 ${pheno_path}/${phenotype}_HP_WGS_UK10K_stand_residuals.txt | cut -f1,3 | awk 'BEGIN{OFS="\t"}{print $1,$1,$2}' > tmp.phenodata
...
do
    file=$(basename $file)
    echo $plink --memory 500 --bfile $input_bfiles/$file --pheno tmp.phenodata --out input/$file --make-bed
done | ~/array 500 rph > phenotypize.command
...
#echo "plotting corrected..."
#hsub 10g -I -q yesterday ~/man_qq_annotate --chr-col chr --pos-col ps --auto-label --pval-col p_score --title "MANOLIS-$phenotype-MAF0.001" --sig-thresh 1e-08 --sig-thresh-line 1e-8 MANOLIS.$phenotype.maf0.001.assoc.txt.gccorrect MANOLIS.$phenotype.maf0.001.assoc.txt.gccorrect
#echo "gzipping corrected..."
#gzip MANOLIS.$phenotype.maf0.001.assoc.txt.gccorrect
cd ..
