...
# DATA PATHS
phenotype=$1
pheno_path="/nfs/t144_helic/Phenotypes/DataExtracts/WGS/Residuals-Plots"
...
#column=$(head -n1 ${pheno_path}/${phenotype}_HELIC_HA_WGS_stand_residuals.txt | tr '\t' '\n' | grep -w -n $phenotype| sed 's/\:.*//')
...
tail -n+2 ${pheno_path}/${phenotype}_HELIC_HA_WGS_stand_residuals.txt | cut -f1,3 | awk 'BEGIN{OFS="\t"}{print $1,$1,$2}' > tmp.phenodata
...
do
    file=$(basename $file)
    echo $plink --memory 2000 --bfile $input_bfiles/$file --pheno tmp.phenodata --out input/$file --make-bed --mac $2
...
do
    echo /nfs/team144/it3/Software_farm3/gemma-0.94/bin/gemma -bfile input/$i -n 1 -notsnp  -maf 0  -miss 1  -km 1 -k /lustre/scratch114/projects/helic/checkpoints/assoc_test/phenotypes_real/$phenotype/matrix/matrix.$phenotype.cXX.txt -lmm 4 -o $i
