...
do
    file=$(basename $file)
    echo $plink --memory 15000 --bed $input_bfiles/$file.bed --bim $input_bfiles/$file.bim --fam $input_fam/$file.fam --pheno tmp.phenodata --out input/$file --make-bed
done | ~ag15/array 20g rph | sed 's/red/green/'> phenotypize.command
...
do
    echo /nfs/users/nfs_s/sh29/software/gemma-0.97/bin/gemma -bfile input/$i -n 1 -notsnp  -maf 0  -miss 1  -km 1 -k $matrix_bfile -lmm 4 -o $i
