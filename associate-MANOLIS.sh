...
do
    file=$(basename $file)
    echo $plink --memory 5000 --bfile $input_bfiles/$file --pheno tmp.phenodata --out input/$file --make-bed
done | ~/array 5g rph > phenotypize.command
...
do
    echo /nfs/team144/it3/Software_farm3/gemma-0.94/bin/gemma -bfile input/$i -n 1 -notsnp  -maf 0  -miss 1  -km 1 -k matrix/matrix.$phenotype.cXX.txt -lmm 4 -o $i
...
hsub 10g -I -q yesterday ~ag15/man_qq_annotate --chr-col chr --pos-col ps --auto-label --pval-col p_score --title "MANOLIS-$phenotype" --sig-thresh 1e-08 --sig-thresh-line 1e-08 MANOLIS.$phenotype.assoc.txt MANOLIS.$phenotype.assoc
